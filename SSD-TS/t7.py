...
def _filter_single_channel(field: np.ndarray, guide: np.ndarray, method: str, radius: int, eps: float) -> np.ndarray:
    """Apply guided or bilateral filtering to a single float32 channel."""
...
def masked_smooth(field: np.ndarray, mask01: np.ndarray, guide: np.ndarray, method: str, radius: int, eps: float) -> np.ndarray:
    """Normalized smoothing inside the mask only."""
    num = _filter_single_channel(field * mask01, guide, method, radius, eps)
    den = _filter_single_channel(mask01, guide, method, radius, eps)
    return num / (den + 1e-6)


def apply_lut_to_lab(img_rgb: np.ndarray, grid: np.ndarray, lut_lab: np.ndarray, lut_order: str, chunk: int) -> np.ndarray:
    """Map RGB -> Lab via LUT."""
    h, w = img_rgb.shape[:2]
...
def apply_lut_to_rgb(img_rgb: np.ndarray, grid: np.ndarray, lut_rgb: np.ndarray, lut_order: str, chunk: int) -> np.ndarray:
    """Map RGB -> RGB via LUT."""
    h, w = img_rgb.shape[:2]
    flat_rgb = img_rgb.reshape(-1, 3).astype(np.float32)
...
def main() -> None:
    ...
    parser.add_argument("--mask_path", type=str, default=None, help="If provided, only replace the masked region")
    parser.add_argument("--mask_invert", action="store_true")
    parser.add_argument("--mask_feather", type=int, default=7, help="Feather radius in pixels; 0 gives a hard mask")
    ...
    if args.mask_path is not None:
        m = read_mask_float(args.mask_path, invert=args.mask_invert, feather=args.mask_feather)
        if args.ms_down > 0:
