...
def parse(opt_path, is_train=True):
    ...
    existing_cvd = os.environ.get("CUDA_VISIBLE_DEVICES", None)
    if existing_cvd is None:
        # 本地/非调度环境：可以尝试设置；若被禁则跳过
        try:
            os.environ["CUDA_VISIBLE_DEVICES"] = gpu_list
            print("export CUDA_VISIBLE_DEVICES=" + gpu_list)
        except PermissionError:
            print("[WARN] PermissionError: cannot set CUDA_VISIBLE_DEVICES in this environment. "
                "Please set it in the shell before launching.")
    else:
        # 调度环境：只读不写，避免 PermissionError
        print(f"[INFO] CUDA_VISIBLE_DEVICES already set by environment: {existing_cvd}. "
            f"Ignore opt.gpu_ids={gpu_ids} for env export.")
