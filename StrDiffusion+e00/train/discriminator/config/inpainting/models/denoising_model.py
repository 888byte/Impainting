...
class DenoisingModel(BaseModel):
    ...
    def optimize_parameters(self, step, timesteps, sde=None):
        ...
        D_fake =self.dis(timesteps.squeeze(), self.state.detach(),S_optimum.detach()) #(t, yt, xt_1) -- False
        errD_fake = F.softplus(D_fake.view(-1))
        errD_fake = errD_fake.mean()
        
        _D_fake =self.dis(timesteps.squeeze(), yt_1_optimum.detach()*self.mask+self.state.detach()*(1-self.mask),S_optimum.detach())
        err_anchor = self.loss_tri(_D_fake,D_real,D_fake)
        
        loss_d = errD_real + errD_fake + err_anchor 
        loss_d.backward()
        self.optimizer_d.step()

        # set log
        self.log_dict["loss"] = loss_d.item()
