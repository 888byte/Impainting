"""
网络构建入口。
define_G 返回：生成器（去噪网络）+ 判别器（对抗训练）。
下方还包含 time-conditioned Discriminator 的实现。
"""
...
#冻结主干网络训练
def freeze_backbone_only_train_brush(netG: nn.Module):
    # 先全冻结
    ...
    for name, p in netG.named_parameters():
        if ("pixel_brushnet" in name) or ("_brush_" in name):
...
def define_G(opt):
    ...
    if use_pb:
        ...
        # 这里 cfg 的 base_ch 推荐与你 UNet 的 nf 相同（更稳）
        ...
        # ✅ 调用 UNet 的接口启用（这样 use_pb=false 时不会创建额外参数）
        netG.enable_pixel_brushnet(cfg)
        if opt["train"].get("freeze_backbone", False):
            freeze_backbone_only_train_brush(netG)
...
class TimestepEmbedding(nn.Module):
    """时间步嵌入：sinusoidal embedding + MLP。"""
...
class Discriminator(nn.Module):
  ...
  def forward(self, t, x_t,S_optimum):
    """t 为时间步嵌入，x_t 为噪声图，S_optimum 为结构引导。"""
