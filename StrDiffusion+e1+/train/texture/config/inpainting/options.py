...
负责：
1) 读取 YAML 配置
2) 展开数据集路径、设置 CUDA_VISIBLE_DEVICES
3) 生成实验输出目录、日志目录
4) 提供 dict -> NoneDict 的安全访问
"""
...
def parse(opt_path, is_train=True):
    ...
    # 兼容受限环境：不允许修改环境变量时给出提示
    try:
        os.environ["CUDA_VISIBLE_DEVICES"] = gpu_list
    except PermissionError:
        print("[WARN] Managed environment forbids setting CUDA_VISIBLE_DEVICES. "
          f"Keep existing CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}")
