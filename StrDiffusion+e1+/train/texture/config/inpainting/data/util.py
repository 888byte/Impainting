"""
数据读写与图像预处理工具集。
包含：路径扫描、lmdb 读取、图像增强、颜色空间转换、imresize 等。
"""
...
def _get_paths_from_lmdb(dataroot):
    """从 lmdb 的 meta_info.pkl 读取图像 key 与分辨率。"""
...
def read_img(env, path, size=None):
    """读取图像（cv2 或 lmdb），返回 float32, HWC, BGR, [0,1]。"""
    if env is None:  # img
...
def rgb2ycbcr(img, only_y=True):
    """RGB -> YCbCr（与 Matlab 实现一致），可选仅输出 Y 通道。"""
...
def bgr2ycbcr(img, only_y=True):
    """BGR -> YCbCr（与 Matlab 实现一致），可选仅输出 Y 通道。"""
    in_img_type = img.dtype
    img.astype(np.float32)
    if in_img_type != np.uint8:
        img *= 255.
...
def imresize(img, scale, antialiasing=True):
    """仿 Matlab 的 bicubic resize（支持 torch/np 输入）。"""
