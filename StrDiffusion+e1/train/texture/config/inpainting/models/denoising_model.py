...
class DenoisingModel(BaseModel):
    ...
    def optimize_parameters(self, step, timesteps, sde=None):
        ...
        S_timestep, S_optimum = self.S_sde.generate_random_states_texture(
            x0=self.S_GT, mu=self.S_LQ * self.mask, timesteps=timesteps
        )
        ...
        # 记录梯度范数（backward 后、step 前）
        gnorm = grad_l2_norm(self.model.module if isinstance(self.model, DataParallel) else self.model)

        self.optimizer.step()

        # （可选）EMA 你定义了但从没 update 过，这里顺便补上
        if hasattr(self, "ema"):
            self.ema.update()

        # ---- 写 TensorBoard 的标量（train.py 会自动 add_scalar）----
        self.log_dict["loss"] = float(loss.detach().item())
        self.log_dict["lr/G"] = get_lr(self.optimizer)

        # timestep 统计
        t = timesteps.float()
        self.log_dict["t/mean"] = float(t.mean().item())
        self.log_dict["t/min"]  = float(t.min().item())
        self.log_dict["t/max"]  = float(t.max().item())

        # mask 覆盖率（0/1 mask 的平均值）
        self.log_dict["mask/ratio"] = float(self.mask.float().mean().item())

        # noise/score 的统计（训练是否正常很有帮助）
        self.log_dict["noise/abs_mean"] = float(noise.detach().abs().mean().item())
        self.log_dict["noise/std"]      = float(noise.detach().std().item())
        self.log_dict["score/abs_mean"] = float(score.detach().abs().mean().item())
        self.log_dict["score/std"]      = float(score.detach().std().item())

        # 梯度范数
        self.log_dict["grad_norm/G"] = float(gnorm)

        # 显存（可选，但你经常 OOM/多卡，这个很关键）
        if torch.cuda.is_available():
            dev = torch.cuda.current_device()
            self.log_dict["gpu/mem_alloc_MB"]   = float(torch.cuda.memory_allocated(dev) / 1024**2)
            self.log_dict["gpu/mem_reserved_MB"] = float(torch.cuda.memory_reserved(dev) / 1024**2)
